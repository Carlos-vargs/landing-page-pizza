...
export default function CardCombos({ data }) {
    ...
    return (
        <Flex
            ...
            data-aos={userScreen > nonAnimatedScreen ? `fade-${data.animation}`: 'none'}
        ...
        >
            <Center flexDirection="column" w="full">
                ...
                <Text fontSize="28px" textTransform="uppercase" fontWeight="600" as="h3" mb="30px">{data.category}</Text>
                <Flex gridGap="30px" flexDirection="column">
                    {
                        data.infoCombo.map(e => <Text key={e.id} >{e.name}</Text>)
                    }
